...
void MyApplication()
{
	...
	string filename("Media/CarSpeedTest1EmptyFrame.jpg");
	...
	else
	{
		...
		//Code below Was Done By Sean Briscoe
		int choice;
		int count =0;
		Mat first_frame;
		Mat background = template_image;
		double aspectRatio;
		double trueAspectRatio = 456/100;
		double area;
		double trueAreaMin = 200;
		double trueAreaMax = 3000;
		int trueX = 1;
		double speed;
		//x represents first frame y represents second frame for comparison i.e. x =54 y = 70
		int x, y,  frameDiff;
		double time,xWidth, yWidth, widthDiff, distX, distY, dist;
		double focal = 1770;
		double trueWidth = 465;
		Rect newTangle;
		...
		{	
			...
			//SPEED CALC FROM HERE ->
			/*
			else if (count == 54) {
				x = count;
				xWidth = newTangle.width;
				...
				y = count;
				frameDiff = y - x;
				yWidth = newTangle.width;
				time = frameDiff * 0.3;
				widthDiff = yWidth - xWidth;
				distX = (focal * trueWidth) / xWidth;
				distY = (focal * trueWidth) / yWidth;
			...
			// TO HERE ->
				...
				Mat img = current_frame;
				Mat blur, grey,bin,bin1;
				Mat car;
				Mat contrast;
				Mat test;
				Mat test2;
				...
				//Isolate the Car
				absdiff(current_frame, template_image, car);
				test = car;
				//Increase the contrast to get more defined edges
				car.convertTo(contrast, -1, 2, 0);
				test2 = contrast;
				
				//Convert to greyscale and Canny threshold
				cvtColor(contrast, grey, COLOR_RGB2GRAY);
				//threshold(grey, bin1, 100, 255, THRESH_BINARY | THRESH_OTSU);
				Canny(grey, bin1, 290, 485, 3);

				
				//Apply closing to ensure plate outline isnt lost
				Mat five_by_five_element(5, 5, CV_8U, Scalar(1));
				morphologyEx(bin1, bin, MORPH_CLOSE, five_by_five_element); 

				//Find all contours and sort them in terms of size smallest to ,largest
				findContours(bin, contours, hierarchy, RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);
				sort(contours.begin(), contours.end(), [](const vector<Point>& contour1, const vector<Point>& contour2) {
					return contourArea(contour1, false) < contourArea(contour2, false);
					});



				//Search through the 20 biggest contours, to speed up processing
				for (int contour_number = contours.size()-20;(contour_number < contours.size()); contour_number++)
				{
					//Red colour for contours
					Scalar colour(0, 0, 255);

					//Create a rotated rectangle around each contour
					RotatedRect min_bounding_rectangle = minAreaRect(contours[contour_number]);
					//Gather rotated rectangle's corner points
					cv::Point2f corners[4];
					min_bounding_rectangle.points(corners);
					
					//Make the rotated rectangle a bounding box
					newTangle = min_bounding_rectangle.boundingRect();
					
					//Compare the aspect ratio of the boundary box to the aspect ratio of the licence plate from specification
					aspectRatio = newTangle.width / newTangle.height;
					//Get the area of bounding box
					area = newTangle.area();

					//Is bounding box aspect ratio the same as the real aspect ratio?
					if (aspectRatio < trueAspectRatio + 1 && aspectRatio > trueAspectRatio -1){
						
						//Is the area of the bounding box between the biggest and smallest plate areas?
						if (area <= trueAreaMax && area>= trueAreaMin) {

							//If above is true draw a bounding box on the image
							for (int i = 0; i < 4; i++) {
								cv::line(car, corners[i], corners[(i + 1) % 4], colour);
				...
				//Show me binary image and boundary box image
				imshow("Plates located", car);
					
				}
				//Code Above was done By Sean Briscoe
